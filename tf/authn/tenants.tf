...
resource "google_identity_platform_tenant_default_supported_idp_config" "public_idp_config" {
  project = local.gcp_project_id
  enabled       = true
  tenant        = google_identity_platform_tenant.public_tenant.name
  idp_id        = "google.com"
  client_id     = "${var.public_tenant_client_id}"
  client_secret = "${var.public_tenant_client_secret}"
}
